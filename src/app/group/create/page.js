...
import createGroup from "/src/app/utils/group"

// Used server action to insert the data
export default function GroupCreationPage() {
    ...
    async function handleCreate (formData) {
        ...
        if(group) {
            setMessage(`The group created: ${group.name} - ${group.join_url}`)
        } else {
    ...
    return (
        <>
            <div className="flex justify-center items-center h-screen bg-gray-100">
                <form action={handleCreate} className="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
